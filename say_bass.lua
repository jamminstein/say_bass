...
local function build_scale(root_pc, steps)
  ...
  for oct = 1,7 do
    for _,s in ipairs(steps) do
      ...
      if n >= BASS_LO-12 and n <= BASS_HI+12 then t[#t+1] = n end
    end
  end
  table.sort(t)
  return t
...
local function on_tick()
  ...
  if armed and (global_tick % TICKS_PER_BEAT == 0) then
    armed=false; recording=true; rec_start=global_tick; current_rec={}
    screen_dirty = true
  end
  if recording then
    local rel = global_tick - rec_start
    if rel >= ticks_per_loop() then finish_recording() end
  end
  if playing then
...
local function draw_splash()
  ...
  local cx=64; local tw,th=9,13; local n_top=6
  local gw=n_top*tw+6; local gy_top=18
  screen.level(11); screen.rect(cx-gw/2, gy_top-6, gw, 7); screen.fill()
  for i=0,n_top-1 do
    local x=cx-gw/2+3+i*tw
    screen.level(15); screen.rect(x,gy_top,tw-2,th); screen.fill()
    screen.level(0);  screen.rect(x+tw-2,gy_top,1,th); screen.fill()
  end
  local n_bot=5; local gy_bot=gy_top+th+4
  screen.level(11); screen.rect(cx-gw/2,gy_bot+th,gw,7); screen.fill()
  for i=0,n_bot-1 do
    local x=cx-gw/2+8+i*tw
    screen.level(15); screen.rect(x,gy_bot,tw-2,th); screen.fill()
    screen.level(0);  screen.rect(x+tw-2,gy_bot,1,th); screen.fill()
  end
  screen.level(15); screen.font_size(8); screen.move(64,56); screen.text_center("SAY BASS")
  screen.level(4);  screen.move(64,63); screen.text_center("press any key")
  screen.update()
end

local function draw_main()
  screen.clear(); screen.aa(0); screen.font_size(8)
  screen.level(15); screen.move(0,7); screen.text("SAY BASS")
  screen.level(6);  screen.move(128,7); screen.text_right(bpm.."bpm  "..loop_bars.."bar")
  if scale_root then
    screen.level(10); screen.move(0,15); screen.text(NOTE_NAMES[scale_root+1].." "..scale_name)
  else
    screen.level(3); screen.move(0,15); screen.text("hum to detect scale")
  end
  local px=84
  screen.level(2); screen.rect(px,8,44,7); screen.fill()
  if detected_hz > 40 then
    ...
    screen.level(11); screen.rect(px,8,math.floor(norm*44),7); screen.fill()
  end
  local sw,sh=13,10; local sy=22
  for i=1,MAX_LOOPS do
    local lp=loops[i]; local bx=(i-1)*(sw+3); local is_sel=(i==selected)
    screen.level(is_sel and 4 or 1); screen.rect(bx,sy,sw,sh); screen.fill()
    if lp.filled then
      screen.level(lp.active and 10 or 4); screen.rect(bx+1,sy+1,sw-2,sh-2); screen.fill()
    end
    if is_sel then
      if recording then screen.level((global_tick//6)%2==0 and 15 or 6)
      elseif armed then screen.level(7)
      else screen.level(5) end
      screen.rect(bx,sy,sw,sh); screen.stroke()
    end
    screen.level(lp.filled and 0 or 5)
    screen.move(bx+sw/2+1,sy+sh-2); screen.text_center(tostring(i))
  end
  local bar_y=36
  screen.level(2); screen.rect(0,bar_y,128,4); screen.fill()
  if recording then
    local prog=math.min((global_tick-rec_start)/ticks_per_loop(),1)
    screen.level(12); screen.rect(0,bar_y,math.floor(prog*128),4); screen.fill()
  end
  local st_y=44
  if recording then screen.level(15); screen.move(0,st_y); screen.text("● REC")
  elseif armed then screen.level(8); screen.move(0,st_y); screen.text("◌ ARMED")
  elseif playing then screen.level(12); screen.move(0,st_y); screen.text("▶ SONG")
  else screen.level(4); screen.move(0,st_y); screen.text("■ STOP") end
  local pr_y,pr_h=47,12; local lp=loops[selected]
  screen.level(1); screen.rect(0,pr_y,128,pr_h); screen.fill()
  if lp.filled and lp.length > 0 then
    for _,ev in ipairs(lp.events) do
      if ev.vel > 0 then
        ...
        screen.level(12); screen.pixel(xp,pr_y+yn); screen.fill()
      end
    end
  end
  screen.level(3); screen.move(0,63); screen.text("K1=play  K2=rec  K3=clr")
  screen.level(6); screen.move(128,63); screen.text_right("E1=bpm E2=bars E3=slot")
  screen.update()
end
...
function key(n,z)
  ...
  elseif n==2 then
    if recording then
      recording=false; armed=false; current_rec={}
      if active_note>=0 then note_off(active_note); active_note=-1 end
    elseif armed then armed=false
    else armed=true end
  elseif n==3 then
    local lp=loops[selected]
...
function enc(n,d)
  if splash then return end
  if n==1 then bpm=util.clamp(bpm+d,40,220); params:set("clock_tempo",bpm)
  elseif n==2 then loop_bars=util.clamp(loop_bars+d,1,8)
  elseif n==3 then selected=util.clamp(selected+d,1,MAX_LOOPS) end
  screen_dirty=true
end
...
function init()
  ...
  clock_id = clock.run(function()
    while true do clock.sync(1/TICKS_PER_BEAT); on_tick() end
  end)
  clock.run(function()
    while true do
      clock.sleep(1/30)
